...
if($_GET){
    ...
    if ($checkResult->num_rows > 0) {
        ...
        echo 'alert("The batch and details you inserted already exist."); location.href="payment10_30.php"';
        echo '</script>';
    } else {
        ...
        if($result) {
            echo '<script language="javascript">';
            echo 'alert("Details Successfully Inserted"); location.href="payment10_30.php"';
            echo '</script>';
        } else {
...
<div style="width: 1374px; height: 659px; position: relative">
  ...
  <div class="tables-scroll">
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td><a href='payment10_30update.php?id=" . $row['id'] . "'><i class='fas fa-edit'></i></a></td>";
            echo "<td>" . $row['code'] . "</td>";
            echo "<td>" . $row['name'] . "</td>";
            echo "<td>" . $row['day1'] . "</td>";
            echo "<td>" . $row['day2'] . "</td>";
                          ...
                          <script>
                              function confirmDelete(id) {
                                  if (confirm("If you 'OK' this details will be surely deleted.")) {
                                      ...
                                      window.location.href = "payment10_30delete.php?id=" + id;
                                  }
                              }
                          </script>
    </div>
  ...
  <button onclick="document.getElementById('id01').style.display='block'"  style="width: 52px; height: 52px; left:79px; top: 151px; position: absolute; background: #3A6050; border-radius: 10px; border: none">
    ...
    <div>
      <form action="payment10_30.php" method="post">
