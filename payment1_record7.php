...
if ($_GET) {
    ...
    if ($checkResult->num_rows > 0) {
        ...
        echo 'alert("The batch and details you inserted is already existed."); location.href="payment1_record7.php"';
        echo '</script>';
    } else {
        ...
        if ($result) {
            echo '<script language="javascript">';
            echo 'alert("Data Successfully Inserted"); location.href="payment1_record7.php"';
            echo '</script>';
        } else {
            echo "Failed";
        }
...
<?php
    ...
    if(isset($_POST['submit2'])) {
        $Search = $_POST['search'];
        $query = "SELECT * FROM record7 WHERE CONCAT (`name`) LIKE '%".$Search."%'";
        $result = available($query);
    }
    else {
        $query = "SELECT * FROM record7";
        $result = available($query);
    }
  ...
  <div id="id01" class="w3-modal">
    <div class="w3-modal-content w3-card-4 w3-animate-zoom" style="max-width:900px">
      ...
      <div class="w3-container w3-border-top w3-padding-16" style="background: #3A6050">
        ...
        <br>
        <center>
          <button type="submit" onclick="document.getElementById('id01').style.display='none'" type="button" class="w3-button w3-green" style=" width: 100px; border-radius: 10px; margin-left: 40px; height: 50px; font-family: sans-serif; font-weight: bold;">Save</button>
          <button onclick="document.getElementById('id01').style.display='none'" type="button" class="w3-button w3-red" style="width: 100px; border-radius: 10px; margin-left: 20px; height: 50px; font-family: sans-serif; font-weight: bold;">Cancel</button>
        </center>
        </form>     
      </div>
...
<div style="width: 1374px; height: 659px; position: relative"> 
  ...
  <!-- background -->
    <!-- <div style="width: 1529px; height: 682px; left: -16px; top: -8px; position: absolute; background: #D9D9D9; border: none"></div> -->
    <!-- green -->
    ...
    <div style="left: 640px; top: 95px; position: absolute; text-align: center; color: #3A6050; font-size: 36px; font-family: sans-serif; font-weight: bold; word-wrap: break-word">1st PAYMENT</div>
  </div>
  ...
  <div class="tables-scroll">
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          echo "<td><a href='payment1_record7update.php?id=" . $row['id'] . "'><i class='fas fa-edit'></i></a> </td>";
                          ...
                          <script>
                              function confirmDelete(id) {
                                  if (confirm("If you 'OK' this details will be surely deleted.")) {
                                      ...
                                      window.location.href = "payment1_record7delete.php?id=" + id;
                                  }
                              }
                          </script>
    </div>
    ...
    <div>
      <form action="payment1_record7.php" method="post">
  ...
  <div style="display: flex; position: relative; top: 617px; left: 705px">
    ...
    <label for="2" style="margin-left: 10px; margin-right: 10px">1</label>
    <a href="payment2_record7.php">
      <button style="width: 50px; font-size: 15px; background: #3A6050; color: white"><b>></b></button>
    </a>
  </div>
